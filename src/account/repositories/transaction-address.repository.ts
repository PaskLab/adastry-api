...
@EntityRepository(TransactionAddress)
export class TransactionAddressRepository extends Repository<TransactionAddress> {
  async exist(txHash: string, address: string): Promise<boolean> {
    const count = await this.createQueryBuilder('transactionAddress')
